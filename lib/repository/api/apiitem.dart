...
class AllApi {
  ...
  Future<TrendingModel> trendingApi() async {
    Response response = await mainHall.FetchFunc(urldiffrent: true,
  ...
  Future<MovieIdModel>movieIdApi({required String id})async{
    Response response = await mainHall.FetchFunc( urldiffrent: true, pathitem: pathdefinesid+id, method: "GET", body: null);
  ...
  Future<SearchMovieModel>searchMovie({required String moviename})async{
    Response response = await mainHall.FetchFunc(urldiffrent: false, pathitem: moviename, method: "GET", body: null);
